...
export const signAgreement = async (req: Request, res: Response) => {
  try {
    ...
    // Apply diagonal watermark to secure uploaded identity cards if provided, then upload to S3
    let watermarkedFront = nicFront || null;
    let watermarkedBack = nicBack || null;

    if (nicFront && nicFront.startsWith('data:image')) {
      const watermarked = await watermarkImage(nicFront);
      watermarkedFront = await uploadToS3(watermarked, 'stayzo/nic-documents');
    }
    if (nicBack && nicBack.startsWith('data:image')) {
      const watermarked = await watermarkImage(nicBack);
      watermarkedBack = await uploadToS3(watermarked, 'stayzo/nic-documents');
    }
